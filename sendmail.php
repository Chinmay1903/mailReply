...
// require "dbcon.php";
$result = $mysqli->query("SELECT template_version FROM `mail_data` WHERE `template_name` =(SELECT `template_name` FROM `templatename` WHERE `template_name`= '".$templatename."')"); //query
while ($row = $result->fetch_assoc()) {
    $rows[] = $row["template_version"];
}
// print_r($result);
// print_r($rows);
$num = $rows[array_rand($rows, 1)];
// echo $num;
// $mysqli->close();

$result = $mysqli->query("SELECT `template_body_text` FROM `mail_data` WHERE `template_name` = '".$templatename."' AND `template_version` = ". $num); //query
